<main role="main" class="col-md-9 ml-sm-auto col-lg-10 pt-3 px-4" id="page-wrapper">
	<div class="row">
		<div class="col-12">
			<form method="POST" action="<?=SITE_PATH?>/?mod=relay&act=insert">
				<div class="form-group">
					<label for="relay-name">Tên thiết bị được điều khiển</label>
					<input type="text" class="form-control" id="relay-name" name="relay_name">
				</div>
				...
				<div class="form-group">
					...
					<select class="form-control" id="device-id" name="device_id">
						...
						<option value="<?= $device['id'] ?>">Id: <?= $device['id'] ?> - Tên: <?= $device['name'] ?></option>
						<? endforeach ?>
					</select>
				</div>
